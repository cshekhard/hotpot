...
class HISPModel(nn.Module):
    ...
    def forward(self, context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, start_mapping, end_mapping, all_mapping, return_yp=False):
        ...
        context_outputs_attn_word = []
        for context_split in context_splits:
            context_out, context_hid = self.encoder_lev1(context_split)
            context_attn_word = self.word_attn(context_out)
            context_outputs_attn_word.append(torch.unsqueeze(context_attn_word, dim=1))
        ...
        output_t,_ = self.rnn_2(output, input_lengths=context_lens)
